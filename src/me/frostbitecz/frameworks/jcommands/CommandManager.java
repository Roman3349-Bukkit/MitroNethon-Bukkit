...
public class CommandManager implements CommandExecutor
{
	...
	public static void registerCommands(JavaPlugin plugin, Object handler)
	{
		for (Method method : handler.getClass().getMethods())
		{
			...
			if (params.length == 2 && CommandSender.class.isAssignableFrom(params[0]) && CommandContext.class.equals(params[1]))
			{
				if (isCommandHandler(method))
				{
					...
					if (plugin.getCommand(annotation.name()) == null)
					{
						...
						{ "" })))
							activeAliases = Lists.newArrayList(annotation.aliases());
						else
							activeAliases = new ArrayList<String>();
						JCommand jCommand = new JCommand(name,description,usageMessage,activeAliases);
						jCommand.setPermission(annotation.permission());
						jCommand.setPermissionMessage(annotation.permissionMessage());
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		...
		String permission = method.getAnnotation(CommandHandler.class).permission();
		int minimumReqArgs = method.getAnnotation(CommandHandler.class).minimumArgs();
		int maximumReqArgs = method.getAnnotation(CommandHandler.class).maximumArgs();
		if (handler != null && method != null)
		{
			if(!sender.hasPermission(permission))
			{
				sender.sendMessage(ChatColor.RED + method.getAnnotation(CommandHandler.class).permissionMessage());
				return true;
			}
			...
			if(arguments.getArgs().size() > maximumReqArgs && maximumReqArgs != -1)
			{
